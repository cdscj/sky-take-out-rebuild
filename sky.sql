...
CREATE TABLE `orders` (
  ...
  `rider_id` bigint DEFAULT NULL COMMENT '骑手用户',
  ...
  `reminder_count` int DEFAULT '0' COMMENT '催单次数',
  `latest_reminder_time` datetime DEFAULT NULL COMMENT '最新催单时间',
  `reminder_status` tinyint(1) DEFAULT '0' COMMENT '催单状态 0未催单 1已催单',
  PRIMARY KEY (`id`)
...
CREATE TABLE `rider_location` (
  ...
  `update_time` datetime DEFAULT NULL COMMENT '更新时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `idx_rider_id` (`rider_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8_bin COMMENT='骑手位置表';